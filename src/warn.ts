...
export class Warn extends WarnCore {
	footerlinks = {
		'เลือกระดับการเตือน': 'WP:UWUL#Levels',
		'การตั้งค่าเตือน': 'WP:TW/PREF#warn',
		'วิธีใช้ Twinkle': 'WP:TW/DOC#warn',
		'ให้ข้อเสนอแนะ': 'WT:TW',
	};
	...
	> = {
		level1: {
			label: '1: หมายเหตุทั่วไป',
			selected: (pref) => pref === 1,
		},
		level2: {
			label: '2: การเตือนเบื้องต้น',
			selected: (pref) => pref === 2,
		},
		level3: {
			label: '3: การเตือนปานกลาง',
			selected: (pref) => pref === 3,
		},
		level4: {
			label: '4: การเตือนขั้นสุดท้าย',
			selected: (pref) => pref === 4,
		},
		level4im: {
			label: '4im: การเตือนครั้งสุดท้าย',
			selected: (pref) => pref === 5,
		},
		singlenotice: {
			label: 'แจ้งเตือนเรื่องเดียว',
			selected: (pref) => pref === 6,
			visible: () => !getPref('combinedSingletMenus'),
		},
		singlewarn: {
			label: 'การเตือนเรื่องเดียว',
			selected: (pref) => pref === 7,
			visible: () => !getPref('combinedSingletMenus'),
		},
		singlecombined: {
			label: 'สารเรื่องเดียว',
			selected: (pref) => pref === 6 || pref === 7,
			visible: () => !!getPref('combinedSingletMenus'),
		},
		kitchensink: {
			label: 'แม่แบบเตือนทั้งหมด',
			selected: (pref) => pref === 10,
		},
		// unsupported
		autolevel: {
			label: 'เลือกระดับอัตโนมัติ (1-4)',
			selected: (pref) => pref === 11,
		},
		custom: {
			label: 'การเตือนที่กำหนดเอง',
	...
	processWarnings() {
		...
		let groupObject: warningLevel['list'] = {
			...
			'พฤติกรรมในบทความ': [],
			'การส่งเสริมและสแปม': [],
			'พฤติกรรมต่อบรรณาธิการคนอื่น': [],
			'การลบแท็กการลบ': [],
			'อื่น ๆ': [],
		};

		let groups: Record<string, warningLevel> = {
			level1: { label: '1: แจ้งทั่วไป', list: $.extend(true, {}, groupObject) },
			level2: { label: '2: ระวัง', list: $.extend(true, {}, groupObject) },
			level3: { label: '3: เตือน', list: $.extend(true, {}, groupObject) },
			level4: { label: '4: เตือนครั้งสุดท้าย', list: $.extend(true, {}, groupObject) },
			level4im: { label: '4im: เตือนครั้งเดียว', list: $.extend(true, {}, groupObject) },
		};
		...
		for (let [subgroupName, templateSet] of obj_entries(messages.levels)) {
			for (let [templateName, templateLevels] of obj_entries(templateSet)) {
				for (let [level, templateData] of obj_entries(templateLevels)) {
					const fullTemplateName = templateName + level.slice('level'.length);
					console.log(
						'Adding template',
						fullTemplateName,
						'to group',
	...
	validateInputs(params) {
		if (params.sub_group === 'uw-username' && !params.article) {
			return 'คุณต้องระบุเหตุผลสำหรับแม่แบบ {{uw-username}}';
		}
	}

	getHistoryRegex(): RegExp {
		return /<!--\s?(Template|แม่แบบ):([uU]w-.*?)\s?-->.*?(\d{1,2}:\d{1,2}, \d{1,2} \w+ \d{4} \(\+07\))/g;
	}
	...
	getInputConfig(template: string) {
		...
		switch (template) {
			case 'uw-agf-sock':
				input.label = 'ชื่อผู้ใช้ของบัญชีอื่น ถ้ามี (ไม่ต้องใส่ User:) ';
				input.className = 'userInput';
				break;
			case 'uw-bite':
				input.label = "ชื่อผู้ใช้ของบัญชีที่ถูก 'กัด' (ไม่ต้องใส่ User:) ";
				input.className = 'userInput';
				break;
			case 'uw-socksuspect':
				input.label = 'ชื่อผู้ใช้ของเจ้าของผู้เชิดหุ่น หากทราบ (ไม่ต้องใส่ User:) ';
				input.className = 'userInput';
				break;
			case 'uw-username':
				input.label = 'ชื่อผู้ใช้ละเมิดนโยบายเนื่องจาก... ';
				break;
			case 'uw-aiv':
				input.label = 'ชื่อผู้ใช้ที่ถูกรายงาน ถ้ามี (ไม่ต้องใส่ User:) ';
				input.className = 'userInput';
				break;
			// no default
		}
	...
	customiseSummaryWithInput(summary: string, input: string, messageData: warning) {
		...
		if (['uw-agf-sock', 'uw-socksuspect', 'uw-aiv'].includes(messageData.template)) {
			return summary + ' ของ [[:ผู้ใช้:' + input + ']]';
		}
